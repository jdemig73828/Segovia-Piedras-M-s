...
// Componente para el Logotipo de Rutabia (Último icono enviado)
...
const App = () => {
  ...
  useEffect(() => {
    ...
    // Inyección dinámica de Favicon con el último icono
  ...
  const allPlaces = useMemo(() => [
    ...
    { id: 56, name: "IGLESIA DE SAN MILLÁN", category: "Historia", coords: "41°18'03.3\"N 3°44'57.0\"W", address: "SEPÚLVEDA", note: "Antiguo templo que forma parte del conjunto monumento de Sepúlveda." },
    ...
    { id: 66, name: "CASA PALACIO DE LOS MARQUESES DE CASTROSERNA", category: "Historia", coords: "41°11'23.5\"N 3°42'54.2\"W", address: "CASTROSERNA DE ARRIBA", note: "Edificio señorial representative de la nobleza segoviana." },
    ...
    <div className="min-h-screen bg-[#fcfcfd] font-sans selection:bg-indigo-100 text-[24px] sm:text-[26px] lg:text-[24px]">
      ...
      <header className="sticky top-0 z-[1000] h-16 bg-white px-4 md:px-6 flex items-center justify-between border-b border-slate-100 shadow-sm overflow-visible text-slate-800">
        ...
        <div className="flex items-center gap-2 md:gap-3 flex-1 justify-end h-full">
            {/* BOTÓN BUSCAR QUE DESPLIEGA LA BARRA DEBAJO */}
            <button 
      ...
      <section className="relative min-h-[340px] flex flex-col items-center justify-center text-center overflow-visible bg-[#5b21b6] px-6 pt-12 pb-8 bg-[#5b21b6]">
        ...
        <div className="relative z-10 w-full max-w-4xl flex flex-col items-center">
          <h2 className="text-4xl md:text-6xl uppercase italic tracking-tighter drop-shadow-2xl leading-none mb-2 text-white">
            <span className="font-black text-white text-white text-white text-white text-white">CREA</span> <span className="font-semibold text-white">TU RUTA</span>
          </h2>
          <p className="text-white text-[14px] md:text-[16px] lg:text-[18px] mb-10 opacity-90 tracking-wide font-light max-w-2xl mx-auto text-balance">
            <span className="font-black text-white text-white text-white text-white text-white text-white text-white">Descubre</span> parajes sorprendentes en <span className="font-black text-white text-white text-white">Segovia</span>
          </p>
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-[2.5rem] p-8 border border-white/20 shadow-2xl relative mb-12 text-slate-800">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 text-slate-800 text-slate-800 text-slate-800">
                <div className="relative w-full sm:w-auto text-left text-slate-800">
                    ...
                    className="w-full sm:w-[220px] flex items-center justify-between gap-6 px-6 py-4 bg-white rounded-2xl shadow-xl hover:scale-105 transition-all group text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800"
                    >
                        <div className="flex items-center gap-3">
                    ...
                    {showCatMenu && (
                    <div className="absolute top-full left-0 mt-3 w-full sm:w-[240px] p-4 bg-white rounded-2xl shadow-2xl border border-slate-100 z-[1005] animate-fade-in flex flex-col gap-2 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                        {[
                ...
                <div className="relative w-full sm:w-auto text-left text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                    <button 
                    onClick={() => { setShowZoneMenu(!showZoneMenu); setShowCatMenu(false); setIsHeaderSearchOpen(false); }}
                    className="w-full sm:w-[220px] flex items-center justify-between gap-6 px-6 py-4 bg-white rounded-2xl shadow-xl hover:scale-105 transition-all group text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800"
                    >
                        <div className="flex items-center gap-3 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                ...
                <div className="flex flex-col items-center gap-2">
                    ...
                    {(currentCategory !== 'Todos' || currentGeoZone !== 'Todos' || searchTerm !== '') && (
                        <button 
                            ...
                            className="flex items-center gap-2 text-white/60 hover:text-white transition-all text-[11px] font-bold uppercase tracking-widest mt-6 group text-white/60 text-white/60 text-white/60 text-white/60 text-white/60 text-white/60 text-white/60 text-white/60 text-white/60 text-white/60 text-white/60 text-white/60 text-white/60 text-white/60"
                        >
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-12 pt-12 text-center min-h-[600px]">
        ...
        <div className="flex flex-col items-center gap-12 my-12 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
            {totalPages > 1 && <PaginationControls />}
            <FavoriteButton />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 text-left text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
          {displayedPlaces.map((p) => (
            <div key={p.id} className={`relative ${categoryBgColors[p.category]} rounded-[2.2rem] p-4 border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 group animate-fade-in flex flex-col h-full overflow-hidden text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800`}>
                <div className="relative z-10 flex flex-col h-full text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                  <div className={`relative h-52 w-full rounded-[1.8rem] overflow-hidden mb-6 flex items-center justify-center ${categoryVisualBgs[p.category]} shadow-inner`}
                    ...
                    {p.image && <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-80 z-0 pointer-events-none text-white text-white text-white text-white text-white text-white text-white text-white text-white"></div>}
                    ...
                    <div className="absolute bottom-5 left-6 z-20 flex flex-col items-start gap-2">
                      ...
                      <div className="px-2.5 py-1 bg-white/95 rounded-lg border border-white shadow-sm text-slate-900 font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold"><p className="text-[10px] font-mono leading-none">{p.coords}</p></div>
                    </div>
                  </div>
                  <div className="px-3 flex-grow flex flex-col justify-between">
                    <div>
                      <h4 className="text-[15px] font-black uppercase mb-1.5 text-slate-800 tracking-tight leading-tight group-hover:text-[#4338ca] transition-colors line-clamp-2 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">{p.name}</h4>
                      <p className="text-[9px] text-slate-400 font-bold uppercase mb-4 flex items-center gap-1.5 leading-none text-slate-400"><MapPin className="w-3 h-3 text-[#4338ca]" /> {p.address}</p>
                      <p className="text-[11px] text-slate-500 italic mb-8 leading-relaxed opacity-80 line-clamp-3 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500">"{p.note}"</p>
                    </div>
                    {/* Botón Ver sitio: Minúsculas y mayor elegancia */}
                    <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(p.coords)}`} target="_blank" rel="noopener noreferrer" className="bg-black text-white py-3.5 rounded-2xl font-black text-[14px] text-center shadow-lg hover:bg-[#4338ca] transition-all block active:scale-95 leading-none text-white text-white text-white text-white text-white text-white text-white text-white text-white">Ver sitio</a>
                  </div>
                </div>
            </div>
          ))}
        ...
        <div className="col-span-full w-screen relative -ml-[50vw] left-1/2 h-[180px] flex items-center justify-center overflow-hidden shadow-inner bg-cover bg-center group"
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-black/85 via-black/30 to-transparent text-white text-white text-white text-white text-white text-white text-white text-white text-white"></div>
            <div className="max-w-7xl mx-auto px-6 md:px-12 w-full text-center relative z-20 text-white text-white text-white text-white text-white text-white text-white text-white text-white">
      ...
      <footer className="relative bg-[#111827] py-20 px-6 overflow-hidden text-center border-t border-white/5 text-slate-300 text-slate-300 text-slate-300 text-slate-300 text-slate-300 text-slate-300 text-slate-300 text-slate-300">
        <div className="absolute inset-0 bg-esgrafiado-pattern opacity-[0.05] pointer-events-none text-slate-300 text-slate-300 text-slate-300 text-slate-300 text-slate-300 text-slate-300 text-slate-300 text-slate-300"></div>
        <div className="relative z-10 max-w-4xl mx-auto text-balance">
          <div className="bg-[#1f2937]/95 backdrop-blur-2xl rounded-[3rem] p-10 md:p-14 border border-white/10 shadow-2xl mb-12">
            <div className="flex justify-center mb-8 text-white text-white text-white text-white text-white text-white text-white">
              <RutabiaLogo size={40} />
            ...
            <h3 className="text-3xl md:text-5xl uppercase italic tracking-tighter leading-none mb-5 text-white">
               <span className="font-black text-white text-white text-white text-white text-white text-white text-white">CREA</span> <span className="font-semibold text-white">TU RUTA</span>
            </h3>

            <div className="space-y-6 text-slate-300 text-slate-300 text-slate-300 text-slate-300 text-slate-300 text-slate-300 text-slate-300 text-slate-300">
              <p className="text-white/40 text-[14px] md:text-[16px] lg:text-[18px] leading-relaxed max-w-lg mx-auto tracking-wide font-light italic text-balance text-white/60 text-white/60 text-white/60 text-white/60 text-white/60">
                <span className="font-black text-white text-white text-white text-white text-white">Descubre</span> parajes sorprendentes en <span className="font-black text-white text-white text-white text-white">Segovia</span>
              </p>
              ...
              <div className="text-white/50 text-[12px] md:text-[14px] leading-relaxed max-w-2xl mx-auto space-y-2 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400">
                ...
                <div className="pt-4 text-slate-400 text-slate-400 text-slate-400">
              ...
              <div className="border-t border-white/5 pt-8 max-w-xl mx-auto text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400"></div>
              
              <div className="text-white/30 text-[12px] md:text-[14px] flex flex-wrap justify-center gap-x-8 gap-y-2 mb-8 font-medium text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400">
              ...
              <div className="flex flex-col items-center gap-3 transition-opacity opacity-60 hover:opacity-100 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400">
                <img src="https://www.gstatic.com/images/branding/product/2x/maps_96dp.png" alt="Google Maps" className="w-8 h-8" />
                <p className="text-white/40 text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400">Powered By Google Maps</p>
              </div>
            </div>
          </div>
          <div className="text-white/30 text-[14px] uppercase tracking-[0.5em] mt-32 font-bold leading-none italic uppercase text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400">© 2026 RUTABIA</div>
        </div>
      </footer>
      ...
      {itinerary && (
          <div className="fixed inset-0 z-[1500] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in text-slate-900" onClick={() => setItinerary(null)}>
              <div className="bg-white rounded-[3rem] w-full max-w-2xl overflow-hidden shadow-2xl border border-white/20 relative text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800" onClick={e => e.stopPropagation()}>
                <button 
                ...
                <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-indigo-50 text-indigo-700 text-indigo-700 text-indigo-700 text-indigo-700 text-indigo-700 text-indigo-700 text-indigo-700 text-indigo-700 text-indigo-700">
                ...
                <div className="p-8 space-y-6 overflow-y-auto max-h-[65vh] pb-12 text-left text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                        ...
                        <div key={p.id} className="relative text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                            {p.kmFromPrev && (
                                <div className="flex flex-col items-center -mt-6 mb-4 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                                    <ArrowDown className="w-5 h-5 text-black mb-1 lg:w-7 lg:h-7 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800" />
                            ...
                            <div className="flex gap-4 items-start p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                                <div className="bg-indigo-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-black flex-shrink-0 text-xs lg:w-10 lg:h-10 lg:text-[14px] text-white text-white text-white text-white">
                                  {idx + 1}
                                </div>
                                <div className="flex-grow text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                                    <div className="flex items-center gap-2 mb-1 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                                    ...
                                    <p className="text-[10px] font-bold text-slate-400 mb-1 uppercase tracking-tight lg:text-[14px] text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400">{p.address}</p>
                                    <p className="text-[11px] text-slate-500 italic mb-3 leading-relaxed lg:text-[15px] text-slate-500 text-slate-500 text-slate-500 text-slate-500">"{p.note}"</p>
      ...
      {showFavsModal && (
          <div className="fixed inset-0 z-[1500] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in text-slate-900" onClick={() => setShowFavsModal(false)}>
              <div className="bg-white rounded-[3rem] w-full max-w-2xl overflow-hidden shadow-2xl border border-white/20 relative text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800" onClick={e => e.stopPropagation()}>
                <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-fuchsia-50 text-fuchsia-700 text-fuchsia-700 text-fuchsia-700 text-fuchsia-700">
                    <h4 className="font-black uppercase italic text-fuchsia-700 flex items-center gap-2 leading-none lg:text-[22px] lg:text-[26px] font-black text-fuchsia-700 text-fuchsia-700 text-fuchsia-700 text-fuchsia-700 text-fuchsia-700"><Heart className="w-5 h-5 fill-current lg:w-7 lg:h-7 lg:w-9 lg:h-9" /> Crear ruta</h4>
                    <button onClick={() => setShowFavsModal(false)} className="p-2 hover:bg-fuchsia-100 hover:text-fuchsia-600 rounded-full transition-all text-fuchsia-300 lg:w-12 lg:h-12 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800"><X className="w-6 h-6 lg:w-7 lg:h-7" /></button>
                </div>
                <div className="p-8 space-y-4 overflow-y-auto max-h-[60vh] text-left text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                    {favPlacesWithDist.length === 0 ? (
                        <div className="text-center py-20 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400">
                      ...
                      <>
                        {favPlacesWithDist.map((p, idx) => (
                            <div key={p.id} className="relative text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                                {p.kmFromPrev && (
                                    <div className="flex flex-col items-center -mt-4 mb-4 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                                ...
                                <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-white transition-all shadow-sm text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800">
                                    <div className="flex items-center gap-4 text-slate-800 flex-1 text-slate-800 text-slate-800">
                                        <div className={`w-3 h-10 rounded-full flex-shrink-0 ${categoryColors[p.category]}`}></div>
                                        <div>
                                            <div className="flex items-center gap-2 mb-1 text-slate-800 text-slate-800 text-slate-800">
                                    ...
                                    <button onClick={() => toggleFavorite(p.id)} className="p-2 text-rose-500 hover:bg-rose-50 rounded-lg transition-all font-black ml-4 text-rose-500 text-rose-500 text-rose-500 text-rose-500">
                        ...
                        <div className="pt-6 border-t border-slate-100 mt-8 text-center px-4 flex flex-col gap-3 text-white text-white">
                            <a href={`https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(favPlacesWithDist[0].coords)}&destination=${encodeURIComponent(favPlacesWithDist[favPlacesWithDist.length-1].coords)}${favPlacesWithDist.length > 2 ? `&waypoints=${favPlacesWithDist.slice(1,-1).map(p => encodeURIComponent(p.coords)).join('|')}` : ''}&travelmode=driving`} 
                               ...
                               className="w-full bg-black text-white py-4 px-10 rounded-2xl font-black text-[14px] uppercase tracking-widest shadow-xl hover:scale-105 transition-all lg:text-[16px] flex items-center justify-center gap-3 text-white text-white text-white text-white text-white">
                ...
                <div className="p-6 bg-slate-50 border-t border-slate-100 text-center text-slate-400 text-slate-400">
                    <p className="text-[9px] uppercase tracking-widest font-bold text-slate-400 lg:text-[13px] lg:text-[17px] text-slate-400 text-slate-400 text-slate-400 text-slate-400">{favorites.length} parajes seleccionados</p>
                </div>
                {/* Botón Cerrar: 32px arriba y 48px abajo */}
      ...
      {randomPlace && (
          <div className="fixed inset-0 z-[1500] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in text-slate-900" onClick={() => setRandomPlace(null)}>
              <div className="bg-white rounded-[3rem] w-full max-w-lg overflow-hidden shadow-2xl border border-white/20 p-10 text-center relative text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800 text-slate-800" onClick={e => e.stopPropagation()}>
                ...
                <h3 className="text-2xl font-black uppercase mb-2 leading-tight text-slate-800 lg:text-[28px] lg:text-[32px] font-black text-slate-800 text-slate-800 text-slate-800">{randomPlace.name}</h3>
                ...
                <p className="text-slate-500 italic text-sm mb-10 leading-relaxed lg:text-[18px] lg:text-[22px] text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500">"{randomPlace.note}"</p>
                <div className="flex flex-col gap-3">
                    <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(randomPlace.coords)}`} 
                       target="_blank" 
                       rel="noopener noreferrer" 
                       className="bg-black text-white py-4 px-10 rounded-2xl font-black text-[14px] shadow-xl hover:scale-105 transition-all lg:text-[16px] lg:text-[20px] flex items-center justify-center gap-3 text-white text-white text-white">
